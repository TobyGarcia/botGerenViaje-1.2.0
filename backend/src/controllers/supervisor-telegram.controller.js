...
export async function registerSupervisorController(request, response) {
  try {
    ...
    if (result.created) await sendSupervisorWelcomeEmail(result);
    return response.status(result.created ? 201 : 200).json({ success: true, data: { registered: true, confirmed: result.confirmed }, message: result.created ? "Registro creado. Revisa tu correo para confirmar tu cuenta." : "Tu cuenta ya fue registrada." });
