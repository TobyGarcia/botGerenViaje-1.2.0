...
export async function getAdminDashboardSummary() {
  ...
  let viajes_recientes = [];
  try {
    const viajesRecientesRes = await databasePool.query(
      `
        SELECT
          v.id_viajes,
          v.folio,
        ...
        INNER JOIN lugares destino ON destino.id_lugares = v.id_destino
        INNER JOIN estados_viaje ev ON ev.id_estado_viaje = v.id_estado_viaje
        ORDER BY v.fecha DESC, v.id_viajes DESC
        LIMIT 10
      `
    );
    viajes_recientes = viajesRecientesRes.rows;
  } catch (err) {
    console.error("Error consultando viajes recientes para resumen:", err);
  }
