...
export async function saveInspection({ idViaje, idConductor, data }) {
  ...
  const now = await databasePool.query(`SELECT ${operationalDateSql}::date AS fecha, EXTRACT(HOUR FROM ${mexicoNowSql}) AS hora`);
  ...
  const result = await databasePool.query(`
    INSERT INTO inspecciones_vehiculares (
      ...
      observaciones_conductor, firma_conductor, estado, requiere_autorizacion_fuera_horario, actualizado_en
    ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10::jsonb,$11,$12,'PENDIENTE_APROBACION',$13,CURRENT_TIMESTAMP)
    ON CONFLICT (id_viajes) DO UPDATE SET combustible=EXCLUDED.combustible,
    ...
    RETURNING *`, [idViaje, context.id_vehiculos, idConductor, fecha, data.combustible,
      ...
      data.observaciones || null, data.firma, Number(hora) < 7 || Number(hora) >= 12]);
