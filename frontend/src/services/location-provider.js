...
async function getTelegramLocation() {
  ...
  if (!manager.isInited && typeof manager.init === "function") {
    await new Promise((resolve) => manager.init(resolve));
  }
  if (manager.isLocationAvailable === false) throw new Error("Telegram LocationManager no está disponible.");
  return new Promise((resolve, reject) => manager.getLocation((location) => location
...
export async function getCurrentLocation() { try { return await getTelegramLocation(); } catch { return getBrowserLocation(); } }
