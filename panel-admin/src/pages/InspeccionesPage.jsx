...
import { decidirAdminInspeccion, descargarAdminInspeccionPdf, getAdminInspeccionDetalle, getAdminInspeccionPdfPreview, getAdminInspecciones } from "../services/api.js";
...
export default function InspeccionesPage({ onPendingChange }) {
  ...
  const [previewUrl, setPreviewUrl] = useState(""); const [previewLoading, setPreviewLoading] = useState(false);
  async function load() {
    setLoading(true);
  ...
  useEffect(()=>{ load(); },[]);
  function closePreview() { if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(""); }
  function closeDetail() { closePreview(); setDetail(null); }
  ...
  async function openPreview() { if (!detail) return; setPreviewLoading(true); setMessage(""); try { closePreview(); const blob = await getAdminInspeccionPdfPreview(detail.id_inspeccion); setPreviewUrl(URL.createObjectURL(blob)); } catch(error) { setMessage(error.message); } finally { setPreviewLoading(false); } }
